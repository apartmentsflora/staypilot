...
// ── Bulgarian voice parser ────────────────────────────────────────────────────
const BG_MONTHS: Record<string,number> = { "януари":1,"февруари":2,"март":3,"април":4,"май":5,"юни":6,"юли":7,"август":8,"септември":9,"октомври":10,"ноември":11,"декември":12 };
const BG_ORDS: Record<string,number> = { "първи":1,"втори":2,"трети":3,"четвърти":4,"пети":5,"шести":6,"седми":7,"осми":8,"девети":9,"десети":10,"единадесети":11,"дванадесети":12,"тринадесети":13,"четиринадесети":14,"петнадесети":15,"шестнадесети":16,"седемнадесети":17,"осемнадесети":18,"деветнадесети":19,"двадесети":20,"двадесет и първи":21,"двадесет и втори":22,"двадесет и трети":23,"двадесет и четвърти":24,"двадесет и пети":25,"двадесет и шести":26,"двадесет и седми":27,"двадесет и осми":28,"двадесет и девети":29,"тридесети":30,"тридесет и първи":31 };

function parseBGDate(text: string, yr: number): string | null {
  const t = text.toLowerCase();
  for (const ord in BG_ORDS) {
    for (const mon in BG_MONTHS) {
      if (t.includes(ord+" "+mon)) return `${yr}-${String(BG_MONTHS[mon]).padStart(2,"0")}-${String(BG_ORDS[ord]).padStart(2,"0")}`;
    }
  }
  for (const mon in BG_MONTHS) {
    const m = t.match(new RegExp("(\\d{1,2})\\s+"+mon));
    if (m) return `${yr}-${String(BG_MONTHS[mon]).padStart(2,"0")}-${String(parseInt(m[1])).padStart(2,"0")}`;
  }
  return null;
}

const ALL_ROOM_CODES = ["39.0.1","1.3","1.3A","1.5","2.4.1","2.4.2","2.4.3","2.5","5.5","41.0.1","41.0.2","1.1","1.2","2.2","41-2","3.1","4.1","4.2"];

function parseVoice(text: string, yr: number) {
  const lower = text.toLowerCase();
  const parsed: any = { name: null, room: null, start: null, end: null, phone: null };

  // Room
  for (const code of ALL_ROOM_CODES) {
    if (lower.includes(code.toLowerCase())) { parsed.room = code; break; }
  }

  // Dates
  const fromM = lower.match(/от\s+(.+?)(?:\s+до\s+|$)/);
  const toM   = lower.match(/до\s+(.+?)(?:\s*$)/);
  if (fromM) parsed.start = parseBGDate(fromM[1], yr);
  if (toM)   parsed.end   = parseBGDate(toM[1], yr);

  // Name — words after "за" or first two capitalised words
  const nameM = text.match(/(?:за|гост)\s+([А-ЯA-Z][а-яa-z]+(?:\s+[А-ЯA-Z][а-яa-z]+)?)/);
  if (nameM) { parsed.name = nameM[1]; }
  else {
    const caps = text.split(" ").filter(w => w.length > 2 && /^[А-ЯA-Z]/.test(w));
    if (caps.length >= 2) parsed.name = caps[0]+" "+caps[1];
    else if (caps.length === 1) parsed.name = caps[0];
  }

  // Phone
  const phoneM = text.match(/((?:\+359|0)[0-9\s]{8,12})/);
  if (phoneM) parsed.phone = phoneM[1].replace(/\s+/g,"");
