...
// Codes now match Beds24 1-to-1 with entrance prefix (41 / 39) + two
// special cases: "41-2" (hyphen) = Apartment, "Двустаен партер" = ground-floor apt.
eq("стая тридесет и девет едно точка три",  matchRoomCode("стая тридесет и девет едно точка три"),       "39.1.3");
eq("стая 39.1.3а",           matchRoomCode("стая 39.1.3а"),                "39.1.3а");
eq("апартамент 39 2 4 1",    matchRoomCode("апартамент 39 2 4 1"),          "39.2.4.1");
...
eq("стая четиридесет и едно точка четири точка едно", matchRoomCode("стая четиридесет и едно точка четири точка едно"),  "41.4.1");
...
}
{
  ...
  has("e6 room",  p, "room",  "39.5.5");
}

console.log("── F. Idiomatic / new phrases ────────────────────────────");
{
  const p = parseT("Значи ъъ резервация за Николай около Коледа за три нощи стая 39.1.5 искат паркинг и детско легло");
  has("f1 name", p, "name", "Николай");
  has("f1 start", p, "start", "2026-12-25");
  has("f1 end",   p, "end",   "2026-12-28");
  has("f1 room",  p, "room",  "39.1.5");
  has("f1 notes", p, "notes", "паркинг; детско легло");
}
{
  const p = parseT("за Гергьовден за двама възрастни и две деца в апартамент 41-2 със закуска");
  has("f2 start",  p, "start",  "2026-05-06");
  has("f2 guests", p, "guests", 4);
  has("f2 room",   p, "room",   "41-2");
  has("f2 notes",  p, "notes",  "със закуска");
}
{
  // Departure-only phrase: parser must NOT invent an arrival.
  // "Двустаен партер" = ground-floor apartment (Beds24 label verbatim as code).
  const p = parseT("семейство заминава утре двустаен партер късен чекаут");
  has("f3 guests", p, "guests", 4);
  has("f3 start",  p, "start",  null);
  has("f3 end",    p, "end",    "2026-04-17");
  has("f3 room",   p, "room",   "Двустаен партер");
  has("f3 notes",  p, "notes",  "късно освобождаване");
}
{
  const p = parseT("двойка от довечера за една нощ стая 41.3 по 80 лева на нощ имейл petar at gmail точка com");
  has("f4 guests", p, "guests", 2);
  has("f4 start",  p, "start",  "2026-04-16");
  has("f4 end",    p, "end",    "2026-04-17");
  has("f4 room",   p, "room",   "41.3");
  has("f4 price",  p, "pricePerNight", 80);
  has("f4 email",  p, "email",  "petar@gmail.com");
}
{
  const p = parseT("настанява се в 15:30 часа и заминава в 11:00 стая 41.1.1");
  has("f5 arr",  p, "arrivalTime",   "15:30");
  has("f5 dep",  p, "departureTime", "11:00");
  has("f5 room", p, "room",          "41.1.1");
}
{
  const p = parseT("до края на месеца резервация стая 41.4.1 за компания с гледка към планина");
  has("f6 end",    p, "end",     "2026-04-30");
  has("f6 guests", p, "guests",  6);
  has("f6 room",   p, "room",    "41.4.1");
